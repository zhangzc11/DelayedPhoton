...
for i in range(0,len(fileNameSigSkim_this)):
	...
	fileThis = TFile(fileNameSigSkim_this[i].replace("private_REMINIAOD/withcut","private_REMINIAOD/withcut_BDT_test"), "READ")
	inputTree = fileThis.Get("DelayedPhoton")
	NEvents = fileThis.Get("NEvents")
	outputFile = TFile(fileNameSigSkim_this[i].replace("private_REMINIAOD/withcut","private_REMINIAOD/skim_BDT_test"),"RECREATE")
	outputFile.cd()	
	NEvents_out = NEvents.Clone()
	outputTree = inputTree.CopyTree(cut_skim)
	NEvents_out.Write()
...
for i in range(0,len(fileNameQCDSkim)):
	...
	fileThis = TFile(fileNameQCDSkim[i].replace("private_REMINIAOD/withcut","private_REMINIAOD/withcut_BDT_test"), "READ")
	inputTree = fileThis.Get("DelayedPhoton")
	NEvents = fileThis.Get("NEvents")
	outputFile = TFile(fileNameQCDSkim[i].replace("private_REMINIAOD/withcut","private_REMINIAOD/skim_BDT_test"),"RECREATE")
	outputFile.cd()	
	NEvents_out = NEvents.Clone()
	outputTree = inputTree.CopyTree(cut_skim_bkg)
	outputTree.Write()
