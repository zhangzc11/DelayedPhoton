...
for i in range(0,len(fileNameGJets)):
	thisFile = root.TFile(fileNameGJets[i].replace('skim_noBDT','skim_smear'))
...
for i in range(0, len(fileNameDataSkim)):
	newFileNameThis = fileNameDataSkim[i].replace("private_REMINIAOD/withcut", "private_REMINIAOD/skim_smear")
	newFileNameThis_GoodLumi = newFileNameThis.replace(".root","_GoodLumi.root")
        print "reweighting Data file: "+newFileNameThis_GoodLumi
	smear_file(newFileNameThis_GoodLumi, hist_weight)
