...
;
		...
		myC->SaveAs(outputDir+"/ZeeTiming/iEeffFit_xtal_"+std::to_string(i)+"_dt_data_CLK_"+label+"_nonCalib.png");		
		...
		myC->SaveAs(outputDir+"/ZeeTiming/iEeffFit_xtal_"+std::to_string(i)+"_dt_MC_CLK_"+label+"_nonCalib.png");		
	...
	myC->SaveAs(outputDir+"/ZeeTiming/TimingReso_xtals_dt_vs_Eeff_sigma_Data_vs_MC_2016_select_CLK_"+label+"_nonCalib.pdf");
	myC->SaveAs(outputDir+"/ZeeTiming/TimingReso_xtals_dt_vs_Eeff_sigma_Data_vs_MC_2016_select_CLK_"+label+"_nonCalib.png");
	myC->SaveAs(outputDir+"/ZeeTiming/TimingReso_xtals_dt_vs_Eeff_sigma_Data_vs_MC_2016_select_CLK_"+label+"_nonCalib.C");
...
void drawTimingMap(TH2F * h2_map, TString label, TString title="time [ns]", float minZ=-5.0, float maxZ=5.0){
	...
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_nonCalib.pdf");
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_nonCalib.png");
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_nonCalib.C");
	...
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_zoom_nonCalib.pdf");
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_zoom_nonCalib.png");
	myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_"+label+"_zoom_nonCalib.C");
...
void TimingMap_2016_nonCalib()
{
	...
	for(int iEta = 0; iEta < 171; iEta++){
		for(int iPhi = 0; iPhi<360; iPhi++){
			for(int idx_neighb = 0; idx_neighb < 6; idx_neighb++){
				...
				float result_singGaus = singGausFit(h1_array_time_data[iEta][iPhi][idx_neighb]);
				if(iPhi == 11 && iEta%9==0) myC->SaveAs(outputDir+"/ZeeTiming/Timing_vs_iEta_iPhi_data_iEta"+std::to_string(iEta)+"_iPhi"+std::to_string(iPhi)+"_ineighb_"+std::to_string(idx_neighb)+"_nonCalib.png");
	...
	//draw the map
	TFile * file_out;
	if(!drawOnly) file_out = new TFile ("IC_maps/IC_average_timing_2016_nonCalib.root","RECREATE");
	else {
		file_out = new TFile ("IC_maps/IC_average_timing_2016_nonCalib.root","READ");
