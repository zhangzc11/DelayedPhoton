...
void drawTimingICMap(TH2F * h2_map, TString label, TString title, float range){
	...
	h2_map->GetZaxis()->SetRangeUser(minZ,maxZ);
        h2_map->Draw("colz");
	myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_all.pdf");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_all.png");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_all.C");
        ...
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_zoom_all.pdf");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_zoom_all.png");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_"+label+"_zoom_all.C");
...
void draw1DICDiff(TH1F * h1_map, TString label, TString title){
        ...
        h1_map->Draw("");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_1DDiff_"+label+"_all.pdf");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_1DDiff_"+label+"_all.png");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_1DDiff_"+label+"_all.C");
        delete myC;
}

void TimingCalibSingleElectron_2016_iter_all()
{

	TFile * file_data = new TFile("/data/zhicaiz/data/Run2Analysis/EcalTiming/ntuples_V4p1_31Aug2018/ElectronTiming_All2016_all.root","READ");
	...
	for(int iIter = 0; iIter < max_iter; iIter++){
		...
		TFile * file_start;
		if(starting_iter >= 0) file_start = new TFile(("IC_maps/IC_map_SingleElectron_2016_all_iter"+std::to_string(starting_iter)+".root").c_str(),"READ");

		TFile * file_out;
		TFile * file_previous;
		if(!drawOnly)  file_out = new TFile(("IC_maps/IC_map_SingleElectron_2016_all_iter"+std::to_string(iIter)+".root").c_str(),"RECREATE");
		else  file_out = new TFile(("IC_maps/IC_map_SingleElectron_2016_all_iter"+std::to_string(iIter)+".root").c_str(),"READ");
		if(iIter > 0) file_previous = new TFile(("IC_maps/IC_map_SingleElectron_2016_all_iter"+std::to_string(iIter-1)+".root").c_str(),"READ");
		...
		drawTimingICMap(h2_IC_map_this, ("data_SingleElectron_iter"+std::to_string(iIter)).c_str(), ("IC timing iter "+std::to_string(iIter)+" / ns").c_str(), 1.0);
	...
	gr_IC_diff_vs_iter->GetYaxis()->SetTitleSize( axisTitleSize );
        ...
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_convergence_SingleElectron_all.pdf");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_convergence_SingleElectron_all.png");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_convergence_SingleElectron_all.C");
	...
	gr_IC_residual_vs_iter->GetYaxis()->SetTitleSize( axisTitleSize );
        ...
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_residual_convergence_SingleElectron_all.pdf");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_residual_convergence_SingleElectron_all.png");
        myC->SaveAs(outputDir+"/ZeeTiming/IC_Timing_vs_iEta_iPhi_residual_convergence_SingleElectron_all.C");
